...
# ソフトウェア管理
永続的なソフトウェアは基本的にNixで管理する。Nixで管理できないソフトウェアは、ユーザの指示がある場合のみ、プロジェクト内の`tools`ディレクトリに配置すること。決してbrewやapt等を利用してはならない。ユーザの指示がある場合でも、brewやapt等を利用する場合は、必ず事前に許可を得ること。
